...
assert.ok(!/errorMsg\.textContent\s*=\s*error\.message/.test(src),
...
assert.ok(/auth\/email-already-in-use[\s\S]{0,200}PENDING_MESSAGE/.test(src),
...
assert.ok(/consent\.checked/.test(src), 'registration must require the consent checkbox');
assert.ok(/consentAt/.test(src), 'registration must record when consent was given');
